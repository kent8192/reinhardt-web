...
```bash
# Currently a pre-release: --version is required. Once 0.1.2 stable ships,
...
<!-- reinhardt-version-sync -->
**Current status:** Reinhardt is at `0.1.2`. From `0.1.0` onward, all
...
reinhardt = { version = "0.1.2", package = "reinhardt-web" }
```
...
```toml
[dependencies]
reinhardt = { version = "0.1.2", package = "reinhardt-web", default-features = false, features = ["full"] }
```
...
```toml
[dependencies]
reinhardt = { version = "0.1.2", package = "reinhardt-web", default-features = false, features = ["minimal"] }
```
...
reinhardt-graphql = "0.1.2"
reinhardt-websockets = "0.1.2"
```
