...
because `cargo audit` reported 7 vulnerability findings. The same command now
uses `.cargo/audit.toml`, matches the GitHub Actions security-audit gate, and
passes with the temporary ignores listed below.

The advisory DB reports one High CVSS finding: `RUSTSEC-2026-0195` has CVSS
7.5. `RUSTSEC-2023-0071` has CVSS 5.9, and the remaining 5 findings do not
...
| [`RUSTSEC-2026-0194`](https://rustsec.org/advisories/RUSTSEC-2026-0194) | `quick-xml` | `0.31.0`, `0.38.4`, `0.39.4` | `>=0.41.0` | legacy `azure_core 0.21`, Azure SDK 1.0 `typespec` XML support, `syntect -> plist 1.9`, and `reinhardt-db` default validator metadata through `phonenumber 0.3.x` | Accepted temporarily. Reinhardt's direct `quick-xml` dependencies are pinned to `0.41.0`; the remaining vulnerable versions are held by upstream dependency constraints. Remove when Azure SDK, `plist`, and `phonenumber` release dependency ranges that admit `quick-xml >=0.41.0`, or when the legacy Azure staticfiles backend or phone-number validator path is isolated from the default graph. |
| [`RUSTSEC-2026-0195`](https://rustsec.org/advisories/RUSTSEC-2026-0195) | `quick-xml` | `0.31.0`, `0.38.4`, `0.39.4` | `>=0.41.0` | legacy `azure_core 0.21`, Azure SDK 1.0 `typespec` XML support, `syntect -> plist 1.9`, and `reinhardt-db` default validator metadata through `phonenumber 0.3.x` | Accepted temporarily. This High-severity advisory shares the same upstream-bound paths as RUSTSEC-2026-0194. Direct Reinhardt dependencies are remediated; remove with the same upstream quick-xml dependency refresh tracked in #5492. |
...
- move upstream-constrained Azure SDK, `plist`, and `phonenumber` paths off
  `quick-xml <0.41.0`;
