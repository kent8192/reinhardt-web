...
The new `submit_async` primitive is additive and generic. It returns
`Result<UseFormAsyncSubmitOutcome<T>, E>`, where the outcome distinguishes
submitted, already-pending, and validation-failed states while `Err(E)` remains
the async operation error. The primitive owns `is_submitting`,
`is_submit_successful`, `submit_error`, validation blocking, and submit
lifecycle callbacks for async submissions. The existing synchronous
