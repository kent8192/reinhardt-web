...
- `Cargo.toml` — declares `crate-type = ["cdylib", "rlib"]` (cdylib for WASM, rlib for the server binary), `default-run = "manage"`, and an `[[bin]] manage` whose `required-features = ["with-reinhardt"]` keep tokio + reinhardt-commands out of the WASM build. It splits dependencies between two `[target.'cfg(...)'.dependencies]` blocks (WASM vs server), and declares the integration test target as `[[test]] name = "integration", required-features = ["with-reinhardt"]`.
- `build.rs` — uses the `cfg_aliases` crate to register two custom cfgs: `wasm` = `all(target_family = "wasm", target_os = "unknown")` and `native` = its negation. You will see these throughout the source as `#[cfg(wasm)]` / `#[cfg(native)]`.
...
- `src/config/`
  ...
  - `urls.rs` — `#[routes(standalone)] pub fn routes() -> UnifiedRouter`. Registers every server function via `.server(|s| s.server_fn(name::marker))`, mounts the admin at `/admin/` via `admin_routes_with_di(Arc::new(configure_admin()))`, and applies the session middleware. App-level routers are mounted automatically by the framework using the `#[url_patterns]` attribute on each app's `server_url_patterns` / `client_url_patterns` function — `src/config/urls.rs` does not need explicit `.mount("/polls/", ...)` calls.
...
fn profile_name() -> String {
    env::var("REINHARDT_ENV").unwrap_or_else(|_| {
        if env::var("CI").is_ok() { "ci".to_string() } else { "local".to_string() }
    })
}
