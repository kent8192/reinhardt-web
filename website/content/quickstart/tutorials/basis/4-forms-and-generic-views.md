...
- [`src/shared/types.rs`](https://github.com/kent8192/reinhardt-web/tree/main/examples/examples-tutorial-basis/src/shared/types.rs) — re-exported generated `*Info` companions plus request DTOs that cross the WASM/native boundary. The request DTO validation rules run *only* on the server.
...
The model-shaped `QuestionInfo`, `ChoiceInfo`, and `UserInfo` companions are generated by `#[model]`; `src/shared/types.rs` only re-exports them. The hand-written request DTOs, `LoginRequest` and `RegisterRequest`, live in the same file. They are normal `serde` payloads, decorated with the **`#[dto]`** attribute macro — `#[dto]` is the convention-driven entry point that wraps `Validate` (and an OpenAPI `Schema`) `derive` behind `cfg(native)` for you, so the per-field `#[validate(...)]` attributes can be written plainly without any `#[cfg_attr(...)]` noise:
...
1. **`#[dto]`** — this single attribute is the convention. It emits `#[cfg_attr(native, derive(Validate, Schema))]` for you so the validator-crate `derive` (and the OpenAPI `Schema` derive) are native-only. On WASM the struct still serialises and deserialises, but it has no `validate()` method and pulls in neither dependency.
