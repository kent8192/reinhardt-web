...
1. **No `// Arrange` / `// Act` / `// Assert` labels** — both tests are well under the 5-line threshold per phase, and the call-site context makes the intent self-evident. The labels become mandatory once the test grows or setup involves more than one statement.
2. **Every assertion exercises a Reinhardt component** — `Choice::build()`, `Question::build()`, `Choice::vote()`, `Question::was_published_recently()`. No `assert_eq!(1 + 1, 2)`-style filler.
