...
`was_published_recently()` and `vote()` are plain `impl` methods. They aren't doing anything Reinhardt-specific — they live on the model because that's where the data lives. We keep them small on purpose: Part 3's `vote` server function is going to load the choice, call `Choice::vote(&mut self)`, then persist the change inside an `atomic(&db, …)` block so two concurrent voters do not race. The helper is the only place the increment is spelled out, which means the future "rate-limit a vote per user" or "audit-log the increment" change has exactly one site to edit.
