...
impl Sign for SigV4Signer {
    fn sign_http_request(
        ...
        #[cfg(feature = "event-stream")]
        {
            ...
            use aws_smithy_eventstream::frame::{DeferredSignerSender, SignMessage};
            ...
            if let Some(signer_sender) = config_bag.load::<DeferredSignerSender>() {
                ...
                let name = operation_config
                    ...
                    .clone()
                    .expect("`Self::signing_params` above would have errored, if name was missing");
                // Workaround for smithy-lang/smithy-rs#4696
                // (tracked in reinhardt-web#5274). Remove this patch when
                // aws-runtime publishes a release with an explicit event-stream
                // signer cast.
                //
                // Ideal implementation (without workaround):
                //   signer_sender.send(Box::new(SigV4MessageSigner::new(...)) as _)
                let signer = Box::new(SigV4MessageSigner::new(
                    _signature,
                    identity.clone(),
                    region,
                    name,
