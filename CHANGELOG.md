...
- *(ci)* add semgrep rules for non-production-ready markers
- *(ci)* improve semgrep nonprod-markers detection precision
- *(ci)* fix false positives in nonprod-markers semgrep rules
- *(ci)* address Copilot review comments on nonprod-markers rules
- *(ci)* reduce false positives in nonprod-markers semgrep rules
