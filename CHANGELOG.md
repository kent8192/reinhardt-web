...
- *(web)* [**breaking**] re-export unified wasm ServerRouter from urls prelude
- *(web)* expose unified ServerRouter at crate root on wasm

### Changed

- *(examples)* drop redundant #[cfg] from twitter server_url_patterns

### Documentation

- *(storages)* update test documentation to reflect wiremock replacement
- *(admin)* remove broken DefaultUser intra-doc links
- *(examples)* update tutorial-basis comment for ServerRouter rename
- update scaffold template and basis tutorial for ServerRouter rename
...
- *(pages)* update trybuild fixtures for develop/0.2.0 API changes
- *(pages)* regenerate component_missing_required_prop stderr snapshot
- *(integration)* fix model_info and custom_manager trybuild fixtures
