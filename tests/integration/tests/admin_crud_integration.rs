...
async fn setup_test_db() -> (
    ...
    let database_url = format!(
        "postgres://postgres:test@localhost:{}/admin_test_db",
        port
    );
...
async fn test_admin_list_view_with_ordering() {
    ...
    let list_view =
        ListView::new("TestUser").with_ordering(vec!["username".to_string()]);
...
async fn test_admin_create_view() {
    ...
    let create_view = CreateView::new("TestUser")
        .with_fields(vec!["username".to_string(), "email".to_string(), "is_active".to_string()])
    ...
    let result = db
        .create::<TestUser>("test_users", data)
        .await;
...
async fn test_admin_count_with_filters() {
    ...
    let active_count = db
        .count::<TestUser>("test_users", active_filter)
        .await;
