...
async fn test_render_complex_nested_data() {
	...
	assert_eq!(
		alice_profile.get("age"),
		Some(&json!(30)),
		"Alice's age must be 30"
	);
	assert_eq!(
		alice_profile.get("city"),
		Some(&json!("Tokyo")),
		"Alice's city must be Tokyo"
	);
	...
	assert_eq!(
		bob_profile.get("age"),
		Some(&json!(25)),
		"Bob's age must be 25"
	);
	assert_eq!(
		bob_profile.get("city"),
		Some(&json!("Osaka")),
		"Bob's city must be Osaka"
	);
...
async fn test_renderer_with_full_context() {
	...
	assert_eq!(
		items[0],
		json!("item1"),
		"The first element must be 'item1'"
	);
	assert_eq!(
		items[1],
		json!("item2"),
		"The second element must be 'item2'"
	);
	assert_eq!(
		items[2],
		json!("item3"),
		"The third element must be 'item3'"
	);
}
...
async fn test_large_data_rendering() {
	...
	assert_eq!(
		items.len(),
		100,
		"The length of the items array must be 100"
	);
	...
	assert_eq!(
		first_item.get("id"),
		Some(&json!(0)),
		"The id of the first element must be 0"
	);
	assert_eq!(
		first_item.get("name"),
		Some(&json!("Item 0")),
		"The name of the first element must be 'Item 0'"
	);
	assert_eq!(
		first_item.get("value"),
		Some(&json!(0)),
		"The value of the first element must be 0"
	);
	...
	assert_eq!(
		last_item.get("id"),
		Some(&json!(99)),
		"The id of the last element must be 99"
	);
	assert_eq!(
		last_item.get("name"),
		Some(&json!("Item 99")),
		"The name of the last element must be 'Item 99'"
	);
	assert_eq!(
		last_item.get("value"),
		Some(&json!(990)),
		"The value of the last element must be 990"
	);
