...
// ============================================================================
// Issue #1706: Auto-Increment Schema Validation Tests
// ============================================================================

/// Test auto_increment with Integer generates correct SERIAL column in PostgreSQL
///
/// **Test Intent**: Verify that auto_increment + Integer creates a SERIAL/identity column
#[rstest]
#[tokio::test]
async fn test_auto_increment_integer_schema(
	#[future] postgres_container: (ContainerAsync<GenericImage>, Arc<PgPool>, u16, String),
) {
	let (_container, pool, _port, url) = postgres_container.await;
	...
	let migration = create_test_migration(
		...
		"0001_auto_inc_int",
		vec![Operation::CreateTable {
			name: leak_str("auto_inc_int_table").to_string(),
			columns: vec![
				create_auto_pk_column("id", FieldType::Integer),
				create_not_null_column("name", FieldType::VarChar(100)),
			],
	...
	// Assert - Verify column is an identity column (GENERATED BY DEFAULT AS IDENTITY)
	let is_identity: String = sqlx::query_scalar(
		"SELECT is_identity FROM information_schema.columns
		 WHERE table_name = $1 AND column_name = $2",
	)
	.bind("auto_inc_int_table")
	.bind("id")
	.fetch_one(pool.as_ref())
	.await
	.expect("Failed to query is_identity");

	assert_eq!(
		is_identity, "YES",
		"Auto-increment Integer column should be an identity column"
	);
	...
	.fetch_one(pool.as_ref())
	.await
	.expect("Failed to query data type");

	assert_eq!(
		data_type, "integer",
		"Auto-increment column should be integer type"
	);
	...
	.fetch_one(pool.as_ref())
	.await
	.expect("Failed to check PK");

	assert!(pk_exists, "Auto-increment column should have PRIMARY KEY");
}

/// Test auto_increment with BigInteger generates correct BIGSERIAL column
///
/// **Test Intent**: Verify that auto_increment + BigInteger creates a BIGSERIAL/bigint identity column
#[rstest]
#[tokio::test]
async fn test_auto_increment_biginteger_schema(
	#[future] postgres_container: (ContainerAsync<GenericImage>, Arc<PgPool>, u16, String),
) {
	let (_container, pool, _port, url) = postgres_container.await;
	...
	let migration = create_test_migration(
		"testapp",
		"0001_auto_inc_bigint",
		vec![Operation::CreateTable {
			name: leak_str("auto_inc_bigint_table").to_string(),
			columns: vec![
				create_auto_pk_column("id", FieldType::BigInteger),
				create_not_null_column("data", FieldType::Text),
			],
			constraints: vec![],
			without_rowid: None,
			interleave_in_parent: None,
	...
	// Assert - Verify the column type is bigint
	let data_type: String = sqlx::query_scalar(
		"SELECT data_type FROM information_schema.columns
		 WHERE table_name = $1 AND column_name = $2",
	)
	.bind("auto_inc_bigint_table")
	.bind("id")
	.fetch_one(pool.as_ref())
	.await
	.expect("Failed to query data type");
	...
	assert_eq!(
		data_type, "bigint",
		"BigInteger auto_increment should be bigint"
	);
	...
	assert_eq!(
		is_identity, "YES",
		"Auto-increment BigInteger column should be an identity column"
	);
}

/// Test auto_increment with SmallInteger generates correct SMALLSERIAL column
///
/// **Test Intent**: Verify that auto_increment + SmallInteger creates a SMALLSERIAL/smallint identity column
#[rstest]
#[tokio::test]
async fn test_auto_increment_smallinteger_schema(
	#[future] postgres_container: (ContainerAsync<GenericImage>, Arc<PgPool>, u16, String),
) {
	let (_container, pool, _port, url) = postgres_container.await;
	...
	let migration = create_test_migration(
		"testapp",
		"0001_auto_inc_smallint",
		vec![Operation::CreateTable {
			name: leak_str("auto_inc_smallint_table").to_string(),
			columns: vec![
				create_auto_pk_column("id", FieldType::SmallInteger),
				create_not_null_column("label", FieldType::VarChar(50)),
			],
			constraints: vec![],
			without_rowid: None,
			interleave_in_parent: None,
	...
	// Assert - Verify the column type is smallint
	let data_type: String = sqlx::query_scalar(
		"SELECT data_type FROM information_schema.columns
		 WHERE table_name = $1 AND column_name = $2",
	)
	.bind("auto_inc_smallint_table")
	.bind("id")
	.fetch_one(pool.as_ref())
	.await
	.expect("Failed to query data type");
	...
	assert_eq!(
		data_type, "smallint",
		"SmallInteger auto_increment should be smallint"
	);
	...
	assert_eq!(
		is_identity, "YES",
		"Auto-increment SmallInteger column should be an identity column"
	);
}

/// Test auto_increment column allows sequential inserts
///
/// **Test Intent**: Verify that auto_increment column auto-generates sequential values
#[rstest]
#[tokio::test]
async fn test_auto_increment_sequential_inserts(
	#[future] postgres_container: (ContainerAsync<GenericImage>, Arc<PgPool>, u16, String),
) {
	let (_container, pool, _port, url) = postgres_container.await;
	...
	let migration = create_test_migration(
		...
		"0001_auto_inc_seq",
		vec![Operation::CreateTable {
			name: leak_str("auto_inc_seq_table").to_string(),
			columns: vec![
				create_auto_pk_column("id", FieldType::Integer),
				create_not_null_column("value", FieldType::VarChar(50)),
			],
			constraints: vec![],
			without_rowid: None,
			interleave_in_parent: None,
	...
	// Act - Insert rows without specifying id
	sqlx::query("INSERT INTO auto_inc_seq_table (value) VALUES ($1)")
		.bind("first")
		.execute(pool.as_ref())
		.await
	...
	sqlx::query("INSERT INTO auto_inc_seq_table (value) VALUES ($1)")
		...
		.execute(pool.as_ref())
		.await
		.expect("Failed to insert third row");

	// Assert - Verify sequential IDs were generated
	let ids: Vec<i32> = sqlx::query_scalar("SELECT id FROM auto_inc_seq_table ORDER BY id")
		.fetch_all(pool.as_ref())
		.await
		.expect("Failed to fetch ids");

	assert_eq!(ids.len(), 3, "Should have 3 rows");
	assert_eq!(ids[0], 1, "First row id should be 1");
	assert_eq!(ids[1], 2, "Second row id should be 2");
	assert_eq!(ids[2], 3, "Third row id should be 3");
}
