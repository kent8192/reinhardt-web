...
use chrono::Utc;
use reinhardt_admin::audit::{AuditAction, AuditLog, AuditLogQuery, AuditLogger, DatabaseAuditLogger};
...
async fn test_database_audit_logger_log() {
    ...
    assert!(inserted_log.id().is_some(), "ID should be set after insertion");
...
async fn test_database_audit_logger_log_minimal() {
    ...
    // Log entry
    let result = logger.log(log).await;
    assert!(result.is_ok(), "Log insertion with minimal fields should succeed");
...
async fn test_database_audit_logger_query_by_timestamp_range() {
    ...
    let results_empty = logger.query(&query_empty).await.expect("Query should succeed");
...
async fn test_database_audit_logger_count_with_filters() {
    ...
    let count_user1 = logger.count(&query_user1).await.expect("Count should succeed");
    ...
    let count_user2 = logger.count(&query_user2).await.expect("Count should succeed");
