...
async fn test_complete_authorization_code_flow(oauth2_with_app: OAuth2Authentication) {
	...
	let token = oauth2_with_app
		.exchange_code(&code, "test_client", "test_secret_12345", "https://example.com/callback")
		.await
...
async fn test_exchange_code_with_invalid_credentials(oauth2_with_app: OAuth2Authentication) {
	...
	let result = oauth2_with_app
		.exchange_code(&code, "test_client", "wrong_secret", "https://example.com/callback")
		.await;
...
async fn test_exchange_code_with_unknown_client(oauth2_with_app: OAuth2Authentication) {
	...
	let result = oauth2_with_app
		.exchange_code(&code, "unknown_client", "any_secret", "https://example.com/callback")
		.await;
...
async fn test_exchange_invalid_code(oauth2_with_app: OAuth2Authentication) {
	...
	let result = oauth2_with_app
		.exchange_code("nonexistent_code", "test_client", "test_secret_12345", "https://example.com/callback")
		.await;
...
async fn test_multiple_codes_same_user(oauth2_with_app: OAuth2Authentication) {
	...
	let token1 = oauth2_with_app
		.exchange_code(&code1, "test_client", "test_secret_12345", "https://example.com/callback")
		.await;
	let token2 = oauth2_with_app
		.exchange_code(&code2, "test_client", "test_secret_12345", "https://example.com/callback")
		.await;
...
async fn test_scope_handling(oauth2_with_app: OAuth2Authentication) {
	...
	let token_no_scope = oauth2_with_app
		.exchange_code(&code_no_scope, "test_client", "test_secret_12345", "https://example.com/callback")
		.await
		.unwrap();
	assert!(
	...
	let token_with_scope = oauth2_with_app
		.exchange_code(&code_with_scope, "test_client", "test_secret_12345", "https://example.com/callback")
		.await
		.unwrap();
	assert_eq!(
...
async fn test_use_case_multiple_concurrent_authorizations() {
	...
	let token_a = auth
		.exchange_code(&code_a, "service_a", "secret_a", "https://example.com/callback")
		.await
		.unwrap();
	let token_b = auth
		.exchange_code(&code_b, "service_b", "secret_b", "https://example.com/callback")
		.await
		.unwrap();
...
async fn test_use_case_refresh_token_presence() {
	...
	let token = auth
		.exchange_code(&code, "refresh_test_client", "refresh_test_secret", "https://example.com/callback")
		.await
		.unwrap();
