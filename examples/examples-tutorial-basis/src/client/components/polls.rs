...
pub fn polls_detail(question_id: i64) -> Page {
	...
	// The `VotingForm` struct generated by `form!` is defined inside the
	// macro's block expression, so it is not nameable as a type annotation
	// on page! parameters from out here. Wrap rendering in an `Rc<dyn Fn>`
	// so we can hand a stable, named type to page!.
	let voting_form_for_view = voting_form.clone();
	let form_renderer: std::rc::Rc<dyn Fn() -> Page> =
		std::rc::Rc::new(move || voting_form_for_view.clone().into_page());

	// Render reactively: the `watch` block re-evaluates whenever
	// `load_detail_signal`'s underlying Signal changes, so the spinner
	// is replaced with the form (or error UI) once the action resolves.
	// Returning a static Page early from this function would have stranded
	// the SPA on the spinner forever (see polls_results for the same shape).
	page!(|
		load_detail_signal: Action<(QuestionInfo, Vec<ChoiceInfo>), String>,
		form_renderer: std::rc::Rc<dyn Fn() -> Page>,
		retry_href: String,
		polls_index_href: String,
		edit_href: String,
		delete_href: String,
		results_href: String,
		add_choice_href: String
	| {
		watch {
...
pub fn question_edit(question_id: i64) -> Page {
	...
	// The `EditQuestionForm` struct generated by `form!` is defined inside the
	// macro's block expression, so it is not nameable as a type annotation
	// on page! parameters from out here. Wrap rendering in an `Rc<dyn Fn>`
	// so we can hand a stable, named type to page!.
	let edit_form_for_view = edit_form.clone();
	let form_renderer: std::rc::Rc<dyn Fn() -> Page> =
		std::rc::Rc::new(move || edit_form_for_view.clone().into_page());

	// Render reactively: drive the spinner / error / form switch from the
	// load_detail Signal via `watch`. Returning early with a static Page
	// (the previous shape) stranded the SPA on the spinner forever because
	// the function only ran once.
	page!(|
		load_detail_signal: Action<(QuestionInfo, Vec<ChoiceInfo>), String>,
	...
	| {
		watch {
			if load_detail_signal.is_pending() {
				...
				// Form error + submit-button state are driven by edit_form's
				// own Signals. We inline them in the outer `watch` (instead of
				// nesting another `watch`) so the outer Fn closure does not
				// need to move-capture each Signal a second time into a child
				// closure — see error E0507 from the previous shape.
				div {
					class: "max-w-4xl mx-auto px-4 mt-12",
					h1 {
						class: "mb-4",
