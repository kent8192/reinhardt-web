...
//! `ClientRouter::merge` is `pub(crate)` in `reinhardt-urls`, so user
//! code cannot combine two SPA routers built by separate
//! `#[url_patterns(..., mode = client)]` registrations. Appending the
//! users routes inline keeps a single `ClientRouter` (and therefore a
//! single `ClientUrlReverser`) that covers every page reachable in the
//! SPA. The names use the fully-qualified `users:<name>` form because
//! the polls router's `with_namespace("polls")` has already been applied
//! by the `#[url_patterns(InstalledApp::polls, ...)]` macro, so further
//! routes added at this layer are stored verbatim.
