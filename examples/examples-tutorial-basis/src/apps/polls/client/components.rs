...
pub fn polls_detail(question_id: i64) -> Page {
	...
	let voting_form = form! {
		name : VotingForm, server_fn : submit_vote, method : Post, state : { loading,
		error }, fields : { question_id : HiddenField { initial : qid.to_string(), },
		choice_id : ChoiceField { widget : RadioSelect, required, label :
		"Select your choice", class : "form-check", choices_from : "choices",
		choice_value : "id", choice_label : "choice_text", }, }, strip_arguments : {
...
pub fn question_new() -> Page {
	let new_form = form! {
		name : NewQuestionForm, server_fn : create_question, method : Post, state : {
		loading, error }, redirect_on_success : "/", fields : { question_text : CharField
		{ label : "Question", placeholder : "What do you want to ask?", max_length : 200,
		class : "form-control", }, }, strip_arguments : { csrf_token :
		::reinhardt::reinhardt_pages::csrf::get_csrf_token().unwrap_or_default(), },
	};
...
pub fn question_edit(question_id: i64) -> Page {
	...
	load_detail.dispatch(qid);
	let edit_form = form! {
		name : EditQuestionForm, server_fn : update_question, method : Post, state : {
		loading, error }, redirect_on_success : "/", fields : { question_id : HiddenField
		{ initial : qid.to_string(), }, question_text : CharField { label : "Question",
		placeholder : "Updated question text", max_length : 200, class : "form-control",
		}, }, strip_arguments : { csrf_token :
		::reinhardt::reinhardt_pages::csrf::get_csrf_token().unwrap_or_default(), },
	};
	{
		let load_detail_for_effect = load_detail.clone();
...
pub fn question_delete_confirm(question_id: i64) -> Page {
	...
	let delete_form = form! {
		name : DeleteQuestionForm, server_fn : delete_question, method : Post, state : {
		loading, error }, redirect_on_success : "/", fields : { question_id : HiddenField
		{ initial : qid.to_string(), }, }, strip_arguments : { csrf_token :
		::reinhardt::reinhardt_pages::csrf::get_csrf_token().unwrap_or_default(), },
	};
...
pub fn choice_new(question_id: i64) -> Page {
	...
	let new_form = form! {
		name : NewChoiceForm, server_fn : create_choice, method : Post, state : {
		loading, error }, redirect_on_success : links::detail(qid), fields : {
		question_id : HiddenField { initial : qid_str, }, choice_text : CharField { label
		: "Choice text", placeholder : "An answer option", required, max_length : 200,
		class : "form-control", }, }, strip_arguments : { csrf_token :
		::reinhardt::reinhardt_pages::csrf::get_csrf_token().unwrap_or_default(), },
	};
	let loading_signal = new_form.loading().clone();
...
pub fn choice_edit(question_id: i64, choice_id: i64) -> Page {
	...
	let cancel_href = links::detail(question_id);
	let edit_form = form! {
		name : EditChoiceForm, server_fn : update_choice, method : Post, state : {
		loading, error }, redirect_on_success : "/", fields : { choice_id : HiddenField {
		initial : cid_str, }, choice_text : CharField { label : "Choice text",
		placeholder : "Updated answer option", max_length : 200, class : "form-control",
		}, }, strip_arguments : { csrf_token :
		::reinhardt::reinhardt_pages::csrf::get_csrf_token().unwrap_or_default(), },
	};
...
pub fn choice_delete_confirm(question_id: i64, choice_id: i64) -> Page {
	...
	let cancel_href = links::detail(question_id);
	let delete_form = form! {
		name : DeleteChoiceForm, server_fn : delete_choice, method : Post, state : {
		loading, error }, redirect_on_success : "/", fields : { choice_id : HiddenField {
		initial : cid_str, }, }, strip_arguments : { csrf_token :
		::reinhardt::reinhardt_pages::csrf::get_csrf_token().unwrap_or_default(), },
	};
	let loading_signal = delete_form.loading().clone();
	let error_signal = delete_form.error().clone();
