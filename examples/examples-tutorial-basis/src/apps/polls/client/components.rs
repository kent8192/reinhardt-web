	...
	let voting_form = form! {
		...
		watch: {
			submit_button: |form| {
					...
					page!(
						| is_loading : bool, back_href : String | { div { class : "mt-3", button {
						type : "submit", class : if is_loading {
						"btn-primary opacity-50 cursor-not-allowed" } else { "btn-primary" },
			...
			error_display: |form| {
					...
					page!(
						| err : Option < String >| { watch { if let Some(e) = err.clone() { div {
						class : "alert-danger mt-3", { format_server_error(& e) } } } } }
					)(err)
				},
		}
