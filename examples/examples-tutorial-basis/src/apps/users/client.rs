...
//!
//! Holds the authentication UI (`components`). Typed URL helpers are now
//! emitted by `#[url_patterns]` directly — call sites resolve URLs via
//! `crate::apps::users::urls::client_router::urls::*` (see issue #4656
//! for the removal of the legacy hand-rolled `links` wrapper). The
//! submodule is gated by the `#[cfg(wasm)]` declaration on
//! `pub mod client;` in `apps/users.rs`, so this aggregator does not need
