...
pub fn follow_button(target_user_id: Uuid, is_following_initial: bool) -> Page {
	...
	#[cfg(wasm)]
	{
		...
		page!(
			| is_following_signal : Signal < bool >, toggle_follow : Action < (), String
			>, toggle_follow_for_error : Action < (), String >| { div { if toggle_follow
			.is_pending() { button { type : "button", class :
			"btn-secondary opacity-50 cursor-not-allowed", disabled : { true },
			aria_label : "Loading", @ click : { let { toggle_follow } = toggle_follow
			.clone(); let { is_following_signal } = is_following_signal.clone(); move |
			_event | { toggle_follow.dispatch((target_user_id, is_following_signal
			.get())); } }, div { class : "flex items-center gap-2", div { class :
			"spinner-sm", } } } } else if is_following_signal.get() { button { type :
			"button", class : "btn-outline group", @ click : { let { toggle_follow } =
			toggle_follow.clone(); let { is_following_signal } = is_following_signal
			.clone(); move | _event | { toggle_follow.dispatch((target_user_id,
			is_following_signal.get())); } }, span { class : "group-hover:hidden",
			"Following" } span { class : "hidden group-hover:inline text-danger",
			"Unfollow" } } } else { button { type : "button", class : "btn-primary", @
			click : { let { toggle_follow } = toggle_follow.clone(); let {
			is_following_signal } = is_following_signal.clone(); move | _event | {
			toggle_follow.dispatch((target_user_id, is_following_signal.get())); } },
			"Follow" } } if toggle_follow_for_error.error().is_some() { div { class :
			"alert-danger mt-2 text-sm", { toggle_follow_for_error.error()
			.unwrap_or_default() } } } } }
