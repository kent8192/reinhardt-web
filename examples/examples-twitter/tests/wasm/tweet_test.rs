...
fn test_paginated_response_serialization_roundtrip() {
    ...
    let tweets = vec![
        TweetInfo::new(uuid::Uuid::nil(), uuid::Uuid::nil(), "user1".to_string(),
        "First tweet".to_string(), 1, 0, "2026-01-01T00:00:00Z".to_string(),),
        TweetInfo::new(uuid::Uuid::nil(), uuid::Uuid::nil(), "user2".to_string(),
        "Second tweet".to_string(), 0, 1, "2026-01-02T00:00:00Z".to_string(),),
    ];
    let response = PaginatedResponse::new(tweets, 10, 1, 2, "/api/tweets");
    let json = serde_json::to_string(&response).unwrap();
    let deserialized: PaginatedResponse<TweetInfo> = serde_json::from_str(&json)
        .unwrap();
