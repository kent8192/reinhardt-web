...
///
/// ```
/// use reinhardt_panel::audit::{AuditLog, AuditAction};
/// use chrono::Utc;
/// use serde_json::json;
///
...
///
/// ```
/// use reinhardt_panel::audit::{AuditLogBuilder, AuditAction};
/// use serde_json::json;
///
...
///
/// ```
/// use reinhardt_panel::audit::{AuditLogger, AuditLog, AuditLogQuery};
/// use async_trait::async_trait;
///
...
///
/// ```
/// use reinhardt_panel::audit::{AuditLogQuery, AuditAction};
...
///
/// ```
/// use reinhardt_panel::audit::{MemoryAuditLogger, AuditLogger, AuditLog, AuditAction, AuditLogQuery};
///
...
impl AuditLogger for DatabaseAuditLogger {
	...
	async fn query(
		...
		if let Some(ref model_name) = query.model_name {
			condition = condition.add(Expr::col(Alias::new("model_name")).eq(model_name.as_str()));
		}

		// Filter by object_id
		if let Some(ref object_id) = query.object_id {
			condition = condition.add(Expr::col(Alias::new("object_id")).eq(object_id.as_str()));
		}
		...
		if let Some(start_date) = query.start_date {
			condition =
				condition.add(Expr::col(Alias::new("timestamp")).gte(start_date.to_rfc3339()));
		}
...
mod tests {
	...
	async fn test_memory_audit_logger_query_with_date_range() {
		...
		let query = AuditLogQuery::builder().start_date(future).build();
