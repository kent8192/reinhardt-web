...
///
/// ```
/// use reinhardt_admin_panel::{ExportConfig, ExportFormat};
///
...
impl ExportResult {
	...
	pub fn new(data: Vec<u8>, mime_type: String, filename: String, row_count: usize) -> Self {
		Self {
			data,
			mime_type,
			filename,
...
impl CsvExporter {
	...
	///
	/// ```
	/// use reinhardt_admin_panel::CsvExporter;
	/// use std::collections::HashMap;
	///
...
impl JsonExporter {
	...
	///
	/// ```
	/// use reinhardt_admin_panel::JsonExporter;
	/// use std::collections::HashMap;
	///
	/// let mut row1 = HashMap::new();
	/// row1.insert("id".to_string(), "1".to_string());
...
///
/// ```
/// use reinhardt_admin_panel::{ExportBuilder, ExportFormat};
/// use std::collections::HashMap;
///
