...
/// * `is_update` - Whether this is an update operation (excludes pk_field from readonly check for creates)
///
/// # Errors
///
...
pub fn validate_mutation_data(
	...
	for (field_name, value) in data {
		...
		// Prevent primary key modification (for both create and update)
		// PK is typically auto-generated by the database
		if field_name == pk_field {
			return Err(AdminError::ValidationError(format!(
				"Primary key field '{}' cannot be set or modified",
				field_name
			)));
		}
...
mod tests {
	...
	fn test_validate_pk_field_on_create() {
		// On create, id should also be rejected (auto-generated by DB)
		let admin = create_test_admin();
		let mut data = HashMap::new();
		data.insert("id".to_string(), serde_json::json!(999));
