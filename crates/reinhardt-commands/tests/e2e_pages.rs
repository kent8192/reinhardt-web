...
async fn startapp_pages_layout_has_urls_submodule() {
	...
	//    bodies are empty (no example route comments inside the function).
	...
	assert!(
		!server_contents.contains(".endpoint(views::"),
		"server_urls.rs must not embed example route calls in the function body:\n{server_contents}"
	);
	...
	assert!(
		!client_contents.contains(".named_route(\"index\","),
		"client_router.rs must not embed example route calls in the function body:\n{client_contents}"
	);
