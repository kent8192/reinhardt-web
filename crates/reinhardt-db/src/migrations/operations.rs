		...
		match self {
			...
			Operation::CreateInheritedTable {
				...
				let join_field = FieldState::new(
					...
					super::FieldType::Custom(format!("INTEGER REFERENCES {}(id)", base_table)),
					false,
				);
...
pub struct ColumnDefinition {
	pub name: String,
	pub type_definition: super::FieldType,
	#[serde(default)]
	pub not_null: bool,
	#[serde(default)]
	pub unique: bool,
...
impl ColumnDefinition {
	...
	pub fn new(name: impl Into<String>, type_def: super::FieldType) -> Self {
		Self {
			name: name.into(),
...
pub fn field_type_string_to_field_type(
	...
	match type_name {
		...
		| "SmallIntegerField"
		| "PositiveSmallIntegerField" => Ok(super::FieldType::Integer),
		"BigIntegerField" | "PositiveBigIntegerField" => Ok(super::FieldType::BigInteger),
		"AutoField" => Ok(super::FieldType::Integer),
		"BigAutoField" => Ok(super::FieldType::BigInteger),
		"SmallAutoField" => Ok(super::FieldType::SmallInteger),
		...
		"CharField" => {
			...
			Ok(super::FieldType::VarChar(max_length))
		}
		"TextField" => Ok(super::FieldType::Text),
		"SlugField" => {
			let max_length = attributes
				.get("max_length")
				.and_then(|v| v.parse::<u32>().ok())
		...
		// Boolean type
		"BooleanField" => Ok(super::FieldType::Boolean),
		"NullBooleanField" => Ok(super::FieldType::Boolean),

		// Date/time types
		"DateField" => Ok(super::FieldType::Date),
		"TimeField" => Ok(super::FieldType::Time),
		"DateTimeField" => Ok(super::FieldType::DateTime),
		"DurationField" => Ok(super::FieldType::BigInteger), // Stored as microseconds

		// Numeric types
		"FloatField" => Ok(super::FieldType::Float),
		"DecimalField" => {
			...
			let scale = attributes
				...
				.and_then(|v| v.parse::<u32>().ok())
				.unwrap_or(2);
			Ok(super::FieldType::Decimal { precision, scale })
		}

		// Binary types
		"BinaryField" => Ok(super::FieldType::Binary),

		// UUID type
		"UUIDField" => Ok(super::FieldType::Uuid),

		// JSON types
		"JSONField" => Ok(super::FieldType::Json),
		...
		"GenericIPAddressField" | "IPAddressField" => {
			...
			Ok(super::FieldType::VarChar(39)) // Max length for IPv6
		}

		// Relationship fields (stored as foreign key reference)
		"ForeignKey" => {
			// ForeignKey is typically stored as integer ID
			Ok(super::FieldType::BigInteger)
		}
		"OneToOneField" => Ok(super::FieldType::BigInteger),
...
impl Operation {
	...
	fn apply_column_type(&self, col_def: &mut ColumnDef, field_type: &super::FieldType) {
		use super::FieldType;
		match field_type {
...
mod tests {
	...
	fn test_create_table_to_statement() {
		let op = Operation::CreateTable {
			...
			columns: vec![
				...
				ColumnDefinition {
					name: "name".to_string(),
					type_definition: super::FieldType::VarChar(100),
					not_null: true,
					unique: false,
					primary_key: false,
					auto_increment: false,
	...
	fn test_add_column_to_statement() {
		let op = Operation::AddColumn {
			...
			column: ColumnDefinition {
				...
				type_definition: super::FieldType::VarChar(255),
				not_null: true,
				unique: false,
				primary_key: false,
				auto_increment: false,
	...
	fn test_column_definition_new() {
		let col = ColumnDefinition::new("id", super::FieldType::Integer);
	...
	fn test_apply_column_type_integer() {
		...
		op.apply_column_type(&mut col, &super::FieldType::Integer);
	...
	fn test_apply_column_type_varchar_with_length() {
		...
		op.apply_column_type(&mut col, &super::FieldType::VarChar(100));
	...
	fn test_apply_column_type_custom() {
		...
		op.apply_column_type(
			&mut col,
			&super::FieldType::Custom("CUSTOM_TYPE".to_string()),
		);
