...
impl<M: Model> Manager<M> {
	...
	pub async fn create_with_conn(
		...
		let (fields, values): (Vec<_>, Vec<_>) = obj
			...
			.map(|(k, v)| {
				...
				let value =
					Self::json_to_sea_value_for_field(v, field_info, model.field_is_none(k));
	...
	pub fn bulk_create_query(&self, models: &[M]) -> Option<InsertStatement> {
		...
		let field_names: Vec<String> = first_obj
			...
			.filter_map(|(name, value)| {
				if name == primary_key && value.is_null() {
					None
				} else {
					Some(name.clone())
				}
	...
	pub async fn bulk_create(
		...
		for chunk in models.chunks(batch_size) {
			let mut statement = self.bulk_create_query(chunk).ok_or_else(|| {
				reinhardt_core::exception::Error::Database(
					"Failed to serialize models for bulk create".to_string(),
				)
			})?;
			if !ignore_conflicts {
