...
impl InsertFromSelectBuilder {
	...
	/// Set RETURNING clause (internal helper for from_select conversion)
	fn with_returning(mut self, returning: Option<Vec<String>>) -> Self {
		self.returning = returning;
		self
	}

	/// Set ON CONFLICT action (internal helper for from_select conversion)
	...
	pub fn build(&self) -> (String, Vec<QueryValue>) {
		...
		// Use SELECT statement as data source
		...
		// No parameters for INSERT FROM SELECT (SELECT handles its own params)
		(sql, Vec::new())
	}
