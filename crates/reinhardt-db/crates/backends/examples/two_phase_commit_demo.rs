...
async fn run_postgres_demo() {
	...
	let database_url =
		env::var("DATABASE_URL").unwrap_or_else(|_| "postgresql://localhost/demo_2pc".to_string());
...
async fn run_mysql_demo() {
	...
	sqlx::query("DROP TABLE IF EXISTS accounts")
		...
		.expect("Failed to drop table");
	sqlx::query(
		"CREATE TABLE accounts (id INT AUTO_INCREMENT PRIMARY KEY, name VARCHAR(255), balance INT)",
	)
	.execute(&pool)
	.await
	.expect("Failed to create table");
