...
impl BaseDatabaseSchemaEditor for PostgreSQLSchemaEditor {
    ...
    /// Override to use PostgreSQL-specific DROP INDEX syntax
    fn drop_index_sql(&self, name: &str) -> String {
        format!("DROP INDEX IF EXISTS {}", self.quote_name(name))
    }
}
...
mod tests {
    ...
    fn test_quote_name() {
        ...
        assert_eq!(editor.quote_name("user\"table"), "\"user\"\"table\"");
    }
    ...
    fn test_quote_value() {
        ...
        assert_eq!(editor.quote_value("test"), "'test'");
        assert_eq!(editor.quote_value("test's"), "'test''s'");
    }
}
