	...
	pub fn to_sql(&self, dialect: &SqlDialect) -> String {
		match self {
			Operation::CreateTable {
				...
				let has_composite_pk = columns.iter().filter(|col| col.primary_key).count() > 1;
...
pub fn field_type_string_to_field_type(
	...
	match type_name {
		...
		| "SmallIntegerField"
		| "PositiveSmallIntegerField" => Ok(crate::FieldType::Integer),
		"BigIntegerField" | "PositiveBigIntegerField" => Ok(crate::FieldType::BigInteger),
		"AutoField" => Ok(crate::FieldType::Integer),
		"BigAutoField" => Ok(crate::FieldType::BigInteger),
		"SmallAutoField" => Ok(crate::FieldType::SmallInteger),
		...
		"CharField" => {
			...
			Ok(crate::FieldType::VarChar(max_length))
		}
		"TextField" => Ok(crate::FieldType::Text),
		"SlugField" => {
			let max_length = attributes
				.get("max_length")
				.and_then(|v| v.parse::<u32>().ok())
		...
		// Boolean type
		"BooleanField" => Ok(crate::FieldType::Boolean),
		"NullBooleanField" => Ok(crate::FieldType::Boolean),

		// Date/time types
		"DateField" => Ok(crate::FieldType::Date),
		"TimeField" => Ok(crate::FieldType::Time),
		"DateTimeField" => Ok(crate::FieldType::DateTime),
		"DurationField" => Ok(crate::FieldType::BigInteger), // Stored as microseconds

		// Numeric types
		"FloatField" => Ok(crate::FieldType::Float),
		"DecimalField" => {
			...
			let scale = attributes
				...
				.and_then(|v| v.parse::<u32>().ok())
				.unwrap_or(2);
			Ok(crate::FieldType::Decimal { precision, scale })
		}

		// Binary types
		"BinaryField" => Ok(crate::FieldType::Binary),

		// UUID type
		"UUIDField" => Ok(crate::FieldType::Uuid),

		// JSON types
		"JSONField" => Ok(crate::FieldType::Json),
		...
		"GenericIPAddressField" | "IPAddressField" => {
			...
			Ok(crate::FieldType::VarChar(39)) // Max length for IPv6
		}

		// Relationship fields (stored as foreign key reference)
		"ForeignKey" => {
			// ForeignKey is typically stored as integer ID
			Ok(crate::FieldType::BigInteger)
		}
		"OneToOneField" => Ok(crate::FieldType::BigInteger),
