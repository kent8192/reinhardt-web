...
impl SchemaDiff {
	...
	fn extract_constraints(&self, table_name: &str) -> Vec<crate::Constraint> {
		let table_schema = match self.target_schema.tables.get(table_name) {
			Some(t) => t,
			None => return Vec::new(),
		};
		...
		// Note: ForeignKey constraints from introspection are not included here
		// They are generated by autodetector based on model definitions
