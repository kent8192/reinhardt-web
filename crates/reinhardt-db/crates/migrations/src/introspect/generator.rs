...
use crate::introspect::config::IntrospectConfig;
use crate::introspect::naming::{column_to_field_name, sanitize_identifier, table_to_struct_name};
use crate::introspect::type_mapping::TypeMapper;
use crate::introspection::{ColumnInfo, DatabaseSchema, TableInfo};
...
impl SchemaCodeGenerator {
	...
	fn generate_model_file(
		...
		let file_name = format!(
			"{}.rs",
			crate::introspect::naming::to_snake_case(&table.name)
		);
	...
	fn generate_mod_file(&self, tables: &[&TableInfo]) -> Result<GeneratedFile> {
		...
		for table in tables {
			let snake_name = crate::introspect::naming::to_snake_case(&table.name);
			let struct_name = table_to_struct_name(
				&table.name,
				self.config.generation.struct_naming_convention(),
			);
	...
	fn generate_field(&self, table: &TableInfo, column: &ColumnInfo) -> Result<TokenStream> {
		...
		if let crate::fields::FieldType::VarChar(len) = &column.column_type {
			let len = *len;
...
mod tests {
	...
	use crate::introspection::{ColumnInfo, TableInfo, UniqueConstraintInfo};
	use std::collections::HashMap;
