...
/// Test concurrent 2PC transactions with isolation
///
/// **Test Intent**: Verify that concurrent 2PC transactions are properly isolated and don't interfere
///
/// **Integration Point**: PostgreSQL transaction isolation + 2PC coordination
///
/// **Not Testing**: Isolation levels, deadlock detection
///
/// **Category**: Concurrent transaction case
#[rstest]
#[tokio::test]
async fn test_concurrent_2pc_transactions(
	...
	// Transaction 1: Debit account 1, credit account 2
	let mut tx1 = pool.begin().await.expect("Failed to begin tx1");
	sqlx::query("UPDATE accounts SET balance = balance - $1 WHERE id = $2")
		.bind(100_i64)
		.bind(account1_id)
		.execute(&mut *tx1)
	...
	// Transaction 2: Debit account 2, credit account 1
	let mut tx2 = pool.begin().await.expect("Failed to begin tx2");
	sqlx::query("UPDATE accounts SET balance = balance - $1 WHERE id = $2")
		.bind(50_i64)
		.bind(account2_id)
		.execute(&mut *tx2)
	...
	// Verify final balances (tx1: -100, tx2: -50 for account 1 = -150 net; should be 850)
	// (tx1: +100, tx2: -50 for account 2 = +50 net; should be 2050)
	let balance1 = get_account_balance(pool.as_ref(), account1_id).await;
	let balance2 = get_account_balance(pool.as_ref(), account2_id).await;

	assert_eq!(
		balance1, 950,
		"Account 1 should have 950 after concurrent transactions"
	);
	assert_eq!(
		balance2, 1950,
		"Account 2 should have 1950 after concurrent transactions"
	);
}
