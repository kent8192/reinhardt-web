...
//! # impl reinhardt_orm::Model for Post {
//! #     type PrimaryKey = i64;
...
//! let post = Post { id: Some(1), title: "Example".to_string() };
    ...
    pub fn allocate_field(&'a self, field: &FieldValue) -> &'a SerializedValue<'a> {
        match field {
            FieldValue::String(s) => {
                ...
                self.value_arena.alloc(SerializedValue::String(allocated_str))
            }
            ...
            FieldValue::Object(map) => {
                ...
                self.value_arena.alloc(SerializedValue::Object(allocated_map))
            }
            FieldValue::Array(arr) => {
                let arena_vec: Vec<_> = arr.iter().map(|v| self.allocate_field(v)).collect();
                let allocated_vec = self.vec_arena.alloc(arena_vec);
                self.value_arena.alloc(SerializedValue::Array(allocated_vec))
            }
        }
    }
