...
use super::fields::{FieldInfo, FieldInfoBuilder};
...
///
/// ```
/// use crate::metadata::{SchemaInferencer, FieldType};
///
...
impl SchemaInferencer {
	...
	///
	/// ```
	/// use crate::metadata::SchemaInferencer;
	///
	/// let inferencer = SchemaInferencer::new();
	/// ```
	pub fn new() -> Self {
	...
	/// ```
	pub fn infer_openapi_schema(&self, type_name: &str) -> FieldSchema {
		let field = self.infer_from_type_name(type_name);
		crate::metadata::generate_field_schema(&field)
	}
}
