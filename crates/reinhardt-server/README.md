...
```toml
[dependencies]
reinhardt = { version = "0.2.0-rc.2", features = ["server"] }

# For WebSocket support:
# reinhardt = { version = "0.2.0-rc.2", features = ["server", "websocket"] }

# For GraphQL support:
# reinhardt = { version = "0.2.0-rc.2", features = ["server", "graphql"] }
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    serve("0.2.0-rc.2:8000", handler).await?;
    Ok(())
}
```
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    server.listen("0.2.0-rc.2:8000".parse()?).await?;
    Ok(())
}
```
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    server.listen("0.2.0-rc.2:9001".parse()?).await?;
    Ok(())
}
```
