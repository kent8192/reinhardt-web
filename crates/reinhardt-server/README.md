...
```toml
[dependencies]
reinhardt = { version = "0.1.3", features = ["server"] }

# For WebSocket support:
# reinhardt = { version = "0.1.3", features = ["server", "websocket"] }

# For GraphQL support:
# reinhardt = { version = "0.1.3", features = ["server", "graphql"] }

# Or use a preset:
# reinhardt = { version = "0.1.3", features = ["standard"] }  # Recommended
# reinhardt = { version = "0.1.3", features = ["full"] }      # All features
```
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    serve("0.1.3:8000", handler).await?;
    Ok(())
}
```
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    server.listen("0.1.3:8000".parse()?).await?;
    Ok(())
}
```
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    server.listen("0.1.3:9001".parse()?).await?;
    Ok(())
}
```
