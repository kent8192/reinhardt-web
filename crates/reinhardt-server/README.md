...
```toml
[dependencies]
reinhardt = { version = "0.2.0-rc.1", features = ["server"] }

# For WebSocket support:
# reinhardt = { version = "0.2.0-rc.1", features = ["server", "websocket"] }

# For GraphQL support:
# reinhardt = { version = "0.2.0-rc.1", features = ["server", "graphql"] }

# Or use a preset:
# reinhardt = { version = "0.2.0-rc.1", features = ["standard"] }  # Recommended
# reinhardt = { version = "0.2.0-rc.1", features = ["full"] }      # All features
```
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    serve("0.2.0-rc.1:8000", handler).await?;
    Ok(())
}
```
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    server.listen("0.2.0-rc.1:8000".parse()?).await?;
    Ok(())
}
```
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    server.listen("0.2.0-rc.1:9001".parse()?).await?;
    Ok(())
}
```
