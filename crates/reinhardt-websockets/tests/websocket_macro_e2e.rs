...
impl WebSocketConsumer for EchoWsConsumer {
	async fn on_connect(&self, _ctx: &mut ConsumerContext) -> TestResult {
		Ok(())
	}
	...
	async fn on_disconnect(&self, _ctx: &mut ConsumerContext) -> TestResult {
		Ok(())
	}
}
...
impl WebSocketConsumer for SessionWsConsumer {
	...
	async fn on_message(&self, _ctx: &mut ConsumerContext, _msg: Message) -> TestResult {
		Ok(())
	}
	async fn on_disconnect(&self, _ctx: &mut ConsumerContext) -> TestResult {
		Ok(())
