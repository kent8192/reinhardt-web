...
impl WebSocketConsumer for ChatWsConsumer {
	...
	async fn on_message(&self, _ctx: &mut ConsumerContext, _msg: Message) -> WebSocketResult<()> {
		Ok(())
	}
...
fn test_endpoint_info_path() {
	assert_eq!(
		<ChatWsConsumer as WebSocketEndpointInfo>::path(),
		"/ws/chat/{room_id}/"
	);
	assert_eq!(
		<NotifWsConsumer as WebSocketEndpointInfo>::path(),
		"/ws/notif/"
	);
}
...
fn test_endpoint_info_name() {
	assert_eq!(
		<ChatWsConsumer as WebSocketEndpointInfo>::name(),
		Some("chat_ws")
	);
	assert_eq!(
		<NotifWsConsumer as WebSocketEndpointInfo>::name(),
		Some("notif_ws")
	);
}
...
fn test_substitute_ws_params_multiple() {
	...
	// Act
	let result =
		reinhardt_websockets::substitute_ws_params(path, &[("org", "acme"), ("repo", "app")]);
	// Assert
