...
impl Cache for MemcachedCache {
        ...
        T: for<'de> Deserialize<'de> + Send,
    {
        ...
        Err(Error::Http(
            "All Memcached servers failed".to_string(),
        ))
    }
    ...
    async fn clear(&self) -> Result<()> {
        ...
        if success_count > 0 {
            ...
            Err(last_error.unwrap_or_else(|| {
                Error::Http("Failed to clear cache on all servers".to_string())
            }))
        }
    }
}
...
mod tests {
    ...
    async fn test_memcached_basic_operations() {
        ...
        // Test has_key
        let exists = cache.has_key("test_key").await.expect("Failed to check key");
        assert!(exists);
    ...
    async fn test_clear_all_servers() {
        ...
        let value1: Option<String> = cache.get("clear_test_1").await.expect("Failed to get key 1");
        let value2: Option<String> = cache.get("clear_test_2").await.expect("Failed to get key 2");
