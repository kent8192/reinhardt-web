...
impl Parse for ProducerArgs {
	fn parse(input: ParseStream) -> syn::Result<Self> {
		...
		Ok(ProducerArgs {
			topic: topic
				.ok_or_else(|| syn::Error::new(Span::call_site(), "missing `topic` argument"))?,
			name: name
				.ok_or_else(|| syn::Error::new(Span::call_site(), "missing `name` argument"))?,
		})
	}
}
...
pub(crate) fn producer_impl(args: TokenStream, input: ItemFn) -> syn::Result<TokenStream> {
	...
	let fn_attrs: Vec<_> = input
		.attrs
		.iter()
		.filter(|a| !a.path().is_ident("producer"))
		.collect();

	// Inner function name
	let inner_fn_name = syn::Ident::new(&format!("__producer_inner_{fn_name}"), fn_name.span());
	...
	let call_args: Vec<TokenStream> = input
		...
		.filter_map(|arg| {
			if let FnArg::Typed(pat_type) = arg {
				...
				let is_inject = pat_type.attrs.iter().any(|a| a.path().is_ident("inject"));
				if is_inject {
					return None;
				}
	...
	fn parse(input: ParseStream) -> syn::Result<Self> {
		...
		Ok(ConsumerArgs {
			topic: topic
				.ok_or_else(|| syn::Error::new(Span::call_site(), "missing `topic` argument"))?,
			group: group
				.ok_or_else(|| syn::Error::new(Span::call_site(), "missing `group` argument"))?,
			name: name
				.ok_or_else(|| syn::Error::new(Span::call_site(), "missing `name` argument"))?,
		})
	}
