//! Attribute macro implementation for `#[shared_model]`
//!
...
//! rustdoc on `crate::shared_model` in `lib.rs` for the user-facing contract.
...
pub(crate) fn shared_model_impl(args: TokenStream, mut input: DeriveInput) -> Result<TokenStream> {
	if !args.is_empty() {
		return Err(syn::Error::new_spanned(
			args,
			"#[shared_model] does not accept arguments in this version",
		));
	}
	...
	let fields = match &mut input.data {
		...
		Data::Enum(_) | Data::Union(_) => {
			return Err(syn::Error::new_spanned(
				&input.ident,
				"#[shared_model] can only be applied to structs",
			));
		}
	};
	...
	// introduce a visible name. `as _` allows multiple `#[shared_model]`
