...
/// Re-export endpoint functions and URL resolver modules from submodules.
///
/// When using multi-file view modules (each endpoint in its own file under
/// `views/`), the URL resolver modules generated by `#[get]`/`#[post]`/etc.
/// live inside the submodules. This attribute generates `pub use submod::*;`
...
/// `views.rs`, this attribute is not needed.
#[proc_macro_attribute]
pub fn export_endpoints(args: TokenStream, input: TokenStream) -> TokenStream {
	export_endpoints::export_endpoints_impl(args.into(), input.into())
		.unwrap_or_else(|e| e.to_compile_error())
		.into()
}
