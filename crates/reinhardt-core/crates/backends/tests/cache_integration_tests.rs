...
use reinhardt_backends::cache::{redis::RedisCache, CacheBackend};
...
async fn test_redis_cache_basic_operations() {
    ...
    // Test exists
    let exists = cache.exists("test_key").await.expect("Failed to check exists");
    assert!(exists);
...
async fn test_redis_cache_ttl_expiration() {
    ...
    let exists = cache.exists("ttl_key").await.expect("Failed to check exists");
    assert!(exists);
...
async fn test_redis_cache_batch_operations() {
    ...
    // Test delete_many
    let deleted = cache.delete_many(&keys).await.expect("Failed to delete many");
    assert_eq!(deleted, 5);
...
async fn test_redis_cache_overwrite_value() {
    ...
    cache.delete("overwrite_key").await.expect("Failed to delete");
}
