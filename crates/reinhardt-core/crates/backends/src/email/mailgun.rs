	...
	fn build_form(&self, email: &Email) -> EmailResult<multipart::Form> {
		...
		for attachment in &email.attachments {
			let part = multipart::Part::bytes(attachment.content.clone())
				...
				.map_err(|e| EmailError::Internal(format!("Failed to set MIME type: {}", e)))?;
...
mod tests {
	...
	fn test_mailgun_backend_new() {
		let backend =
			MailgunBackend::new("test-api-key".to_string(), "test-domain.com".to_string());
	...
	async fn test_mailgun_build_form_text() {
		let backend = MailgunBackend::new("test-key".to_string(), "test-domain.com".to_string());
