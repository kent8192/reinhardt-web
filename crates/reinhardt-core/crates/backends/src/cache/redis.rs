...
impl RedisCache {
    ...
    async fn get_connection(
        &self,
    ) -> CacheResult<deadpool_redis::Connection> {
        self.pool
            .get()
            .await
    ...
    fn convert_error(e: RedisError) -> CacheError {
        match e.kind() {
            ...
            redis::ErrorKind::TypeError => {
                CacheError::Serialization(format!("Type error: {}", e))
            }
...
impl CacheBackend for RedisCache {
    ...
    async fn set_many(&self, items: &[(String, Vec<u8>)], ttl: Option<Duration>) -> CacheResult<()> {
        if items.is_empty() {
            return Ok(());
        }
