...
fn main() {
	...
	assert_eq!(admin.list_display(), vec!["id", "user_id", "bio", "location", "created_at"]);
	...
	assert_eq!(admin.readonly_fields(), vec!["id", "created_at", "updated_at"]);
	assert_eq!(admin.fields(), Some(vec!["user_id", "bio", "avatar_url", "location", "website"]));
