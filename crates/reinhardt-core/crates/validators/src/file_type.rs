...
impl FileTypeValidator {
    ...
    pub fn validate_mime_type(&self, mime_type: &str) -> ValidationResult<()> {
        ...
        if let Some(ref allowed) = self.allowed_mime_types {
            ...
            let is_allowed = allowed
                .iter()
                .any(|mime| mime.to_lowercase() == mime_lower);
...
mod tests {
    ...
    fn test_documents_only_mime_types() {
        ...
        assert!(validator.validate_mime_type("application/pdf").is_ok());
        assert!(validator.validate_mime_type("application/msword").is_ok());
        assert!(validator.validate_mime_type("application/vnd.openxmlformats-officedocument.wordprocessingml.document").is_ok());
        assert!(validator.validate_mime_type("application/vnd.ms-excel").is_ok());
        assert!(validator.validate_mime_type("application/vnd.openxmlformats-officedocument.spreadsheetml.sheet").is_ok());
        assert!(validator.validate_mime_type("application/vnd.ms-powerpoint").is_ok());
        assert!(validator.validate_mime_type("application/vnd.openxmlformats-officedocument.presentationml.presentation").is_ok());
    ...
    fn test_mime_type_with_parameters() {
        ...
        assert!(validator.validate_mime_type("text/html; charset=utf-8").is_ok());
    }
