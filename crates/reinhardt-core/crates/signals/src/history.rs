...
//!
//! ```
//! use reinhardt_signals::history::{SignalHistory, HistoryConfig};
...
///
/// ```
/// use reinhardt_signals::history::HistoryEntry;
/// use serde::{Deserialize, Serialize};
///
/// #[derive(Clone, Serialize, Deserialize)]
/// struct Event {
...
///
/// ```
/// use reinhardt_signals::history::HistoryConfig;
...
///
/// ```
/// use reinhardt_signals::history::HistoryStats;
///
...
mod tests {
    ...
    fn test_history_entry() {
        let entry = HistoryEntry::new(TestEvent {
            id: 1,
    ...
    async fn test_history_errors_only() {
        ...
        signal.connect(|_event| async move {
            Err(SignalError::new("Test error"))
        });

        // Send some events (all will fail)
        for i in 0..3 {
            let _ = history.send(TestEvent {
                id: i,
                message: "test".to_string(),
            }).await;
        }
