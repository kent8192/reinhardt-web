...
//!
//! ```
//! use reinhardt_signals::throttling::{ThrottleConfig, ThrottleStrategy, SignalThrottle};
...
///
/// ```
/// use reinhardt_signals::throttling::ThrottleStrategy;
///
...
///
/// ```
/// use reinhardt_signals::throttling::{ThrottleConfig, ThrottleStrategy};
/// use std::time::Duration;
///
/// let config = ThrottleConfig::new()
...
impl<T> ThrottleState<T> {
    ...
    fn can_emit(&mut self, config: &ThrottleConfig) -> bool {
        match config.strategy {
            ...
            ThrottleStrategy::TokenBucket { refill_rate } => self.can_emit_token_bucket(refill_rate),
