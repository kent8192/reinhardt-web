...
//!
//! ```
//! use reinhardt_signals::visualization::{SignalGraph, SignalNode, SignalEdge};
//!
...
impl SignalGraph {
    ...
    ///
    /// ```
    /// use reinhardt_signals::visualization::SignalGraph;
    ///
            ...
            if let Some(signal_node) = self.nodes.get(&signal_id) {
                ...
                for (i, receiver_id) in receivers.iter().enumerate() {
                    if let Some(receiver_node) = self.nodes.get(receiver_id) {
                        ...
                        let critical = if receiver_node.is_critical { " ⚠" } else { "" };
