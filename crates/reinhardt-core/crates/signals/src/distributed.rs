...
///
/// ```
/// use reinhardt_signals::distributed::DistributedEvent;
...
impl<T> DistributedEvent<T> {
    ...
    /// ```
    pub fn new(signal_name: impl Into<String>, payload: T, source_service: impl Into<String>) -> Self {
...
///
/// ```
/// use reinhardt_signals::distributed::InMemoryBroker;
///
/// let broker = InMemoryBroker::new();
/// ```
pub struct InMemoryBroker {
    channels: Arc<RwLock<std::collections::HashMap<String, Vec<Arc<dyn Fn(Vec<u8>) -> Result<(), SignalError> + Send + Sync>>>>>,
}
