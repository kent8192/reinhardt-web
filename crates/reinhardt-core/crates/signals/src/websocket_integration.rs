...
///
/// ```
/// use reinhardt_signals::websocket_integration::WebSocketMessage;
/// use serde_json::json;
///
...
impl WebSocketSignalBridge {
	...
	///
	/// ```
	/// use reinhardt_signals::websocket_integration::{WebSocketSignalBridge, MockWebSocketClient};
	/// use std::sync::Arc;
	///
	/// let bridge = WebSocketSignalBridge::new();
		...
		signal.connect(move |instance| {
			...
			async move {
				...
				for client in clients_read.values() {
					if client.is_connected() {
						let _ = client.send_message(json.clone());
					}
				}
...
///
/// ```
/// use reinhardt_signals::websocket_integration::{WebSocketSignalBridge, TypedWebSocketBroadcaster};
///
/// let bridge = WebSocketSignalBridge::new();
	...
	T: Serialize + DeserializeOwned + Send + Sync + 'static,
{
	...
	///
	/// ```
	/// use reinhardt_signals::websocket_integration::{WebSocketSignalBridge, TypedWebSocketBroadcaster, MockWebSocketClient};
	/// use std::sync::Arc;
	///
	/// let bridge = WebSocketSignalBridge::new();
	/// let client = Arc::new(MockWebSocketClient::new("client-1"));
