...
///
/// ```
/// use reinhardt_signals::transaction::TransactionContext;
///
...
///
/// ```
/// use reinhardt_signals::transaction::on_begin;
///
/// let signal = on_begin();
/// ```
...
///
/// ```
/// use reinhardt_signals::transaction::on_commit;
///
...
///
/// ```
/// use reinhardt_signals::transaction::on_rollback;
///
...
///
/// ```
/// use reinhardt_signals::transaction::on_savepoint;
///
...
///
/// ```
/// use reinhardt_signals::transaction::on_savepoint_release;
///
    ...
    async fn test_nested_transaction_signals() {
        ...
        on_savepoint().connect(move |ctx| {
            ...
            async move {
                e.lock().push(format!(
                    "savepoint:{}:depth:{}",
                    ctx.savepoint_name.as_ref().map(|s| s.as_str()).unwrap_or(""),
                    ctx.savepoint_depth
                ));
                Ok(())
            }
