...
//!
//! ```
//! use reinhardt_signals::persistence::{PersistentSignal, MemoryStore};
...
///
/// ```
/// use reinhardt_signals::persistence::{SignalStore, StoredSignal};
/// use reinhardt_signals::error::SignalError;
/// use async_trait::async_trait;
///
/// struct CustomStore;
///
...
impl<T: Send + Sync + Clone + 'static> PersistentSignal<T> {
    ...
    /// List stored signals with pagination
    pub async fn list(&self, limit: usize, offset: usize) -> Result<Vec<StoredSignal<T>>, SignalError> {
...
mod tests {
    ...
    async fn test_memory_store_basic() {
        let store = MemoryStore::new();
        let event = StoredSignal::new(1, "test".to_string(), TestEvent {
            id: 1,
    ...
    async fn test_memory_store_max_size() {
        ...
        // Store 5 events
        for i in 1..=5 {
            let event = StoredSignal::new(i, "test".to_string(), TestEvent {
                id: i as i32,
