...
///
/// ```rust
/// use reinhardt_contenttypes::persistence::ContentTypeModel;
///
...
pub trait ContentTypePersistenceBackend: Send + Sync {
    ...
    async fn get(&self, app_label: &str, model: &str) -> Result<Option<ContentType>, PersistenceError>;

    /// Get a content type by ID
    async fn get_by_id(&self, id: i64) -> Result<Option<ContentType>, PersistenceError>;

    /// Get or create a content type
    async fn get_or_create(&self, app_label: &str, model: &str) -> Result<ContentType, PersistenceError>;
...
impl ContentTypePersistenceBackend for ContentTypePersistence {
    async fn get(&self, app_label: &str, model: &str) -> Result<Option<ContentType>, PersistenceError> {
        let stmt = Query::select()
            ...
            .cond_where(Condition::all().add(
                Expr::col(Alias::new("model")).binary(BinOper::Equal, Expr::val(model))
            ))
            .to_owned();
        let sql = stmt.to_string(SqliteQueryBuilder);
        let sql_leaked: &'static str = Box::leak(sql.into_boxed_str());
