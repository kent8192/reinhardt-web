...
use sea_query::{Alias, BinOper, Condition, Expr, ExprTrait, Func, Order, Query as SeaQuery, SqliteQueryBuilder};
#[cfg(feature = "database")]
...
impl ContentTypeQuery {
    ...
    pub fn filter_app_label(mut self, app_label: impl Into<String>) -> Self {
        self.filters.push(ContentTypeFilter::AppLabel(app_label.into()));
        self
    }
    ...
    fn build_query(&self) -> String {
        let mut query = SeaQuery::select()
            .columns([Alias::new("id"), Alias::new("app_label"), Alias::new("model")])
        ...
        for filter in &self.filters {
            let condition = match filter {
                ContentTypeFilter::AppLabel(app_label) => {
                    Condition::all().add(Expr::col(Alias::new("app_label")).binary(BinOper::Equal, Expr::val(app_label)))
                }
                ContentTypeFilter::Model(model) => {
                    Condition::all().add(Expr::col(Alias::new("model")).binary(BinOper::Equal, Expr::val(model)))
                }
                ContentTypeFilter::Id(id) => {
                    Condition::all().add(Expr::col(Alias::new("id")).binary(BinOper::Equal, Expr::val(*id)))
                }
            };
    ...
    pub async fn all(&self) -> Result<Vec<ContentType>, PersistenceError> {
        ...
        for row in rows {
            let id: i64 = row.try_get("id").map_err(|e| {
                PersistenceError::DatabaseError(format!("Invalid id: {}", e))
            })?;
    ...
    pub async fn count(&self) -> Result<i64, PersistenceError> {
        ...
        let sql_leaked: &'static str = Box::leak(sql.into_boxed_str());
        let row = sqlx::query(sql_leaked).fetch_one(&*self.pool).await.map_err(|e| {
            PersistenceError::DatabaseError(format!("Failed to count: {}", e))
        })?;

        let count: i64 = row.try_get(0).map_err(|e| {
            PersistenceError::DatabaseError(format!("Invalid count: {}", e))
        })?;
