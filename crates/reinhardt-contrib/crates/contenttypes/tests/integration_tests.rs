...
mod multi_db_tests {
    ...
    async fn test_multi_db_isolated_caches() {
        ...
        let ct2 = manager.get("db2", "isolated", "Model").await.expect("Failed to query");
    ...
    async fn test_multi_db_load_all_with_cache() {
        ...
        let all = manager.load_all("primary").await.expect("Failed to load all");
        assert_eq!(all.len(), 2);
...
mod orm_integration_tests {
    ...
    async fn test_query_count_and_exists() {
        ...
        let tx = ContentTypeTransaction::new(pool.clone());
        tx.create("count_test", "A").await.expect("Failed to create");
        tx.create("count_test", "B").await.expect("Failed to create");
        tx.create("count_test", "C").await.expect("Failed to create");
