...
mod redis_integration {
	use reinhardt_query::nosql::redis::{
		command::CommandStatementBuilder, resp::RespCommand, string::StringCommand,
		transaction::Transaction, zset::ZSetCommand,
	};
	use rstest::*;
	...
	async fn test_set_get_roundtrip() {
		...
		// Act
		let set_resp = send_recv(
			&mut conn,
			&StringCommand::set("hello", "world").ex(60).build(),
		)
		.await;
	...
	async fn test_zadd_zrange_roundtrip() {
		...
		let zrange_resp = send_recv(
			&mut conn,
			&ZSetCommand::zrange("leaderboard", 0, -1).build(),
		)
		.await;
