...
mod tests {
	...
	fn test_mget_command() {
		...
		assert_eq!(
			cmd.args(),
			&[b"MGET".to_vec(), b"key1".to_vec(), b"key2".to_vec(),]
		);
	}
	...
	fn test_incrby_command() {
		...
		assert_eq!(
			cmd.args(),
			&[b"INCRBY".to_vec(), b"counter".to_vec(), b"5".to_vec(),]
		);
	}
	...
	fn test_decrby_command() {
		...
		assert_eq!(
			cmd.args(),
			&[b"DECRBY".to_vec(), b"counter".to_vec(), b"3".to_vec(),]
		);
	}
	...
	fn test_set_basic() {
		...
		assert_eq!(
			cmd.args(),
			&[b"SET".to_vec(), b"mykey".to_vec(), b"myvalue".to_vec(),]
		);
	}
