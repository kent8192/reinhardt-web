...
unsafe extern "C" {
	fn tree_sitter_reinhardt_head() -> *const ();
}

/// The tree-sitter language function for the Reinhardt `head!` DSL grammar.
pub const LANGUAGE: LanguageFn = unsafe { LanguageFn::from_raw(tree_sitter_reinhardt_head) };
...
mod tests {
	...
	#[test]
	fn head_dsl_basic_structure() {
		// Arrange
		let mut parser = tree_sitter::Parser::new();
		parser.set_language(&super::LANGUAGE.into()).unwrap();
		let source = r#"|| { title { "Page" } }"#;
	...
	#[test]
	fn head_dsl_with_block_comment() {
		// Arrange
		let mut parser = tree_sitter::Parser::new();
		parser.set_language(&super::LANGUAGE.into()).unwrap();
		let source = r#"|| { /* comment */ title { "Page" } }"#;
