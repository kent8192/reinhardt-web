    ...
    pub fn compress(
        ...
        match algorithm {
            CompressionAlgorithm::Gzip { level } => {
                ...
                let mut encoder =
                    GzEncoder::new(Vec::new(), Compression::new(*level.min(&9) as u32));
            ...
            CompressionAlgorithm::Brotli { quality } => {
                ...
                brotli::BrotliCompress(&mut reader, &mut output, &brotli::enc::BrotliEncoderParams {
                    quality,
                    ..Default::default()
                })
                .map_err(|e| CompressionError::CompressionFailed(e.to_string()))?;
...
mod tests {
    ...
    async fn test_min_size_filter() {
        ...
        let result = renderer.render(&small_data, Some(&context)).await.unwrap();
        let uncompressed = JSONRenderer::new()
            .render(&small_data, None)
            .await
            .unwrap();
