...
fn test_performance_simple_template() {
	...
	// Compile-time (Tera)
	let template = UserTemplate::new("Test User".to_string(), "test@example.com".to_string(), 25);
	...
	println!(
		"\n=== Simple Template Performance (10 variables, {} iterations) ===",
		ITERATIONS
	);
...
fn test_performance_list_template() {
	...
	for i in 0..LIST_SIZE {
		list_html.push_str(&format!("<li>User {} (user{}@test.com)</li>", i, i));
	}
	...
	println!(
		"\n=== List Template Performance ({} items, {} iterations) ===",
		LIST_SIZE, ITERATIONS
	);
	println!("Tera (Embedded):   {:?}", compile_time_duration);
	println!("Runtime (Phase 2): {:?}", runtime_duration);
...
fn test_memory_characteristics() {
	...
	let tera_template =
		UserTemplate::new("Memory Test".to_string(), "memory@test.com".to_string(), 30);
...
fn test_end_to_end_comparison() {
	...
	println!(
		"\n=== End-to-End Rendering Comparison ({} iterations) ===",
		ITERATIONS
	);

	// Tera
	let template = UserTemplate::new("John Doe".to_string(), "john@example.com".to_string(), 35);
