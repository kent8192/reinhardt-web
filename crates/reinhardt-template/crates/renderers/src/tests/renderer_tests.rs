...
mod json_renderer_tests {
	...
	async fn test_render_array() {
		...
		let array = parsed.as_array().expect("Output must be an array");
...
mod json_formatting_tests {
	...
	async fn test_long_form_rendering() {
		...
		// Validate metadata
		assert_eq!(parsed.get("total"), Some(&json!(3)), "total must be 3");
		assert_eq!(parsed.get("page"), Some(&json!(1)), "page must be 1");
	}
}
