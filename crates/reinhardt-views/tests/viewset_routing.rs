...
async fn model_viewset_list_returns_ok() {
	...
	assert!(!response.body.is_empty(), "list should return a response body");
}
...
async fn model_viewset_retrieve_returns_ok() {
	...
	// Assert
	assert_eq!(response.status, StatusCode::OK);
	assert!(!response.body.is_empty(), "retrieve should return a response body");
}
...
async fn model_viewset_create_returns_created() {
	...
	assert!(!response.body.is_empty(), "create should return a response body");
}
...
async fn model_viewset_update_returns_ok() {
	...
	// Assert
	assert_eq!(response.status, StatusCode::OK);
	assert!(!response.body.is_empty(), "update should return a response body");
}
...
async fn model_viewset_partial_update_returns_ok() {
	...
	// Assert
	assert_eq!(response.status, StatusCode::OK);
	assert!(!response.body.is_empty(), "partial_update should return a response body");
}
...
async fn model_viewset_destroy_returns_no_content() {
	...
	assert!(response.body.is_empty(), "destroy should return empty body for 204");
}
