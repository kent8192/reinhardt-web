...
//! Model-backed forms also implement `reinhardt_pages::FormPageSource`, so a
//! configured `FormServerMutation` can render its attached runtime with `page()`.
//! This presentation shares the mutation's validation and dispatch lifecycle;
//! the existing standalone `into_page()` remains available.
//!
