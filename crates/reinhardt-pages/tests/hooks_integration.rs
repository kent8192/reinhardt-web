...
fn test_hooks_property_memo_deterministic() {
	proptest!(
		| (input in - 10000i32..10000i32) | { let signal = Signal::new(input); let
		signal_clone = signal.clone(); let memoized = use_memo(move || { signal_clone
		.get() * 2 }); let result1 = memoized.get(); let result2 = memoized.get();
		prop_assert_eq!(result1, result2); prop_assert!(result1 == input * 2 || result1
		== 0 * 2); }
	);
}
