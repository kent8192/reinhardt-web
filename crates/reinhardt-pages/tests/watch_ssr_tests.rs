...
fn test_watch_ssr_for_loop(list_signal: Signal<Vec<String>>) {
	...
	let view = page!(
		| items : Signal < Vec < String >>| { ul { for { item } in items.get().iter() {
		li { { item.clone() } } } } }
	)(items.clone());
	let html = view.render_to_string();
...
fn test_watch_ssr_unicode() {
	...
	let view = page!(
		| content : Signal < String >| { div { { content.get() } } }
...
fn test_watch_ssr_data_attributes(counter_signal: Signal<i32>) {
	let counter = counter_signal.clone();
	let view = page!(
		| counter : Signal < i32 >| { div { span { data_count : counter.get()
		.to_string(), data_active : "true", "Counter element" } } }
	)(counter.clone());
	let html = view.render_to_string();
	assert!(
...
fn test_watch_ssr_empty_content() {
	...
	let view = page!(
		| empty : Signal < String >| { div { class : "wrapper", { empty.get() } } }
	)(empty.clone());
	let html = view.render_to_string();
	assert!(
...
fn test_watch_ssr_with_renderer(ssr_renderer: SsrRenderer, string_signal: Signal<String>) {
	let signal = string_signal.clone();
	let view = page!(
		| signal : Signal < String >| { div { p { { signal.get() } } } }
	)(signal.clone());
...
fn test_watch_ssr_deeply_nested(bool_signal_true: Signal<bool>) {
	let signal = bool_signal_true.clone();
	let view = page!(
		| signal : Signal < bool >| { div { class : "level-1", div { class : "level-2",
		div { class : "level-3", if signal.get() { div { class : "level-4", span {
		"Deep content" } } } } } } }
	)(signal.clone());
	let html = view.render_to_string();
	assert!(
