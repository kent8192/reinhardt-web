...
fn test_watch_with_for_loop(list_signal: Signal<Vec<String>>) {
	...
	let view = page!(
		| signal : Signal < Vec < String >>| { ul { for { item } in signal.get().iter() {
		li { { item.clone() } } } } }
	)(signal.clone());
	match &view {
		Page::Element(el) => {
...
fn test_watch_complex_state_machine() {
	...
	let view = page!(
		| loading : Signal < bool >, error : Signal < Option < String >>, data : Signal <
		Vec < String >>| { div { class : "container", if error.get().is_some() { div {
		class : "error", { error.get().unwrap_or_default() } } } if loading.get() { div {
		"Loading..." } } else if data.get().is_empty() { div { "No data" } } else { ul {
		for { item } in data.get().iter() { li { { item.clone() } } } } } } }
