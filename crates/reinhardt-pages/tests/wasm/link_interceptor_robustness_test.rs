...
fn home_page() -> Page {
    PageElement::new("div").attr("id", "route-home").child("HOME").into_page()
}
fn clusters_page() -> Page {
    PageElement::new("div").attr("id", "route-clusters").child("CLUSTERS").into_page()
}
...
async fn link_interceptor_resolves_anchor_from_nested_element_target() {
    ...
    let history = web_sys::window().expect("window").history().expect("history");
    let _ = history.replace_state_with_url(&JsValue::NULL, "", Some("/"));
    ClientLauncher::new("#app").router(build_router).launch().expect("launch");
    ...
    assert_ne!(
        site.as_deref(), Some("test_sentinel"),
    ...
    assert_eq!(
        site.as_deref(), Some("notify_observers"),
