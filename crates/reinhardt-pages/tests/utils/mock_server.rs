...
impl MockServer {
    ...
    pub fn handle_request(
        ...
        if self.log_requests {
            eprintln!(
                "[MockServer] {} {} (headers: {:?}, body: {:?})", method.as_str(), path,
                headers, body
            );
        }
    ...
    pub fn assert_requested_once(&self, method: Method, path: &str) {
        ...
        assert_eq!(
            count, 1, "Expected exactly one request to {} {} but {} were made", method
            .as_str(), path, count
        );
    }
    ...
    pub fn assert_not_requested(&self, method: Method, path: &str) {
        assert_eq!(
            self.request_count_for(method, path), 0,
            "Expected no requests to {} {} but some were made", method.as_str(), path
        );
    }
...
mod tests {
    ...
    fn test_mock_response_json() {
        ...
        assert!(
            response.headers.contains(& ("Content-Type".to_string(), "application/json"
            .to_string()))
        );
    }
    #[test]
