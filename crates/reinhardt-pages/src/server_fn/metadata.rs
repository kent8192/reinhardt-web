...
/// Wire kind for one client-visible server-function argument.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub enum ServerFnArgumentKind {
	/// A scalar value encoded by the selected server-function codec.
	Json,
	/// A required multipart file part.
	File,
	/// An optional multipart file part.
	OptionalFile,
}

/// Compile-time metadata for one client-visible server-function argument.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub struct ServerFnArgumentMetadata {
	/// Rust parameter identifier and multipart part name.
	pub name: &'static str,
	/// Wire representation selected for this argument.
	pub kind: ServerFnArgumentKind,
}
...
pub trait ServerFnMetadata: 'static {
	...
	/// Ordered metadata for client-visible arguments.
	const ARGUMENTS: &'static [ServerFnArgumentMetadata] = &[];

	/// Whether request framing is inferred as multipart from file arguments.
	const USES_MULTIPART: bool = false;
...
/// Exposes one positional server-function argument and its name marker type.
...
/// Proves a server function has exactly `COUNT` client-visible arguments.
pub trait ServerFnArgumentCount<const COUNT: usize>: ServerFnMetadata {}
