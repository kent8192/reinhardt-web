...
///         }
///
///         fn handle(req: Request) -> impl Future<Output = Result<Bytes, Bytes>> + Send {
///             super::__server_fn_handler_login(req)
///         }
///     }
/// }
/// ```
pub trait ServerFnRegistration: ServerFnMetadata + Send + Sync {
	...
	/// Handle this server function without requiring boxed future dispatch.
	///
	/// Generated `#[server_fn]` registrations override this method with their
	/// concrete handler future. Manual registrations can keep relying on the
	/// default compatibility implementation backed by [`Self::handler`].
	fn handle(
		req: Request,
	) -> impl std::future::Future<Output = std::result::Result<Bytes, Bytes>> + Send {
		async move { (Self::handler())(req).await }
	}
}
