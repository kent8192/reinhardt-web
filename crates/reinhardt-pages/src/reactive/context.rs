...
//!
//! ```text
//! Component Tree              Context Stack
//! ┌─────────────────┐         ┌─────────────────┐
//! │ App             │         │ Theme: "dark"   │
//! │ ┌─────────────┐ │         ├─────────────────┤
//! │ │ ThemeProvider │ ────────│ User: {id: 1}   │
//! │ │ ┌─────────┐ │ │         └─────────────────┘
//! │ │ │ UserCtx │ │ │
//! │ │ │ ┌─────┐ │ │ │
//! │ │ │ │Child│ │ │ │  ← use_context accesses stack
//! │ │ │ └─────┘ │ │ │
//! │ │ └─────────┘ │ │
//! │ └─────────────┘ │
//! └─────────────────┘
...
//!
//! ```ignore
//! use reinhardt_pages::reactive::context::{Context, create_context, provide_context, get_context};
//!
...
///
/// ```ignore
/// use reinhardt_pages::reactive::context::{Context, create_context};
///
...
impl<T: 'static> std::fmt::Debug for Context<T> {
...
///
/// ```ignore
/// use reinhardt_pages::reactive::context::create_context;
///
...
///
/// ```ignore
/// use reinhardt_pages::reactive::context::{Context, provide_context};
///
...
///
/// ```ignore
/// use reinhardt_pages::reactive::context::{Context, provide_context, get_context};
///
/// let ctx: Context<i32> = Context::new();
/// provide_context(&ctx, 42);
///
...
mod tests {
	...
	fn test_nested_contexts() {
		...
		assert_eq!(get_context(&ctx), Some("outer".to_string()));

		provide_context(&ctx, "inner".to_string());
		assert_eq!(get_context(&ctx), Some("inner".to_string()));

		remove_context::<String>(&ctx);
		assert_eq!(get_context(&ctx), Some("outer".to_string()));
	...
	fn test_multiple_contexts() {
		...
		provide_context(&ctx2, "hello".to_string());

		assert_eq!(get_context(&ctx1), Some(42));
		assert_eq!(get_context(&ctx2), Some("hello".to_string()));
	}
