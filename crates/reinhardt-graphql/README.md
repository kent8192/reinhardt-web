...
```toml
[dependencies]
reinhardt = { version = "0.1.0-rc.28", features = ["graphql"] }
...
reinhardt = { version = "0.1.0-rc.28", features = ["graphql", "di"] }

# With gRPC transport
reinhardt = { version = "0.1.0-rc.28", features = ["graphql", "grpc"] }
```
...
#[tokio::main]
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    let mut client = GraphQlServiceClient::connect("http://0.1.0-rc.28:50051").await?;
