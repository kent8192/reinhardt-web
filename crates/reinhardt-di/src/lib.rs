...
pub enum DiError {
	...
	#[cfg(feature = "params")]
	#[error(
		"Extractor `{extractor}` requires HTTP request data, but no ParamContext is attached to the InjectionContext"
	)]
	MissingParamContext {
		/// Name of the extractor that triggered the failure (`"Path"`,
		/// `"Query"`, `"Json"`, …) for diagnostic output.
	...
	ParamExtraction(Box<crate::params::ParamError>),
}

#[cfg(feature = "params")]
impl DiError {
	/// Convert a [`ParamError`](crate::params::ParamError) into a `DiError`,
	...
	pub fn from_param_error(err: crate::params::ParamError) -> Self {
		match err {
			// Authentication failures MUST surface as DiError::Authentication
			// so the handler returns HTTP 401, not 500.
			crate::params::ParamError::Authentication(msg) => DiError::Authentication(msg),
			other => DiError::ParamExtraction(Box::new(other)),
		}
	}
}
...
impl From<DiError> for reinhardt_core::exception::Error {
	fn from(err: DiError) -> Self {
		match err {
			...
			DiError::Authorization(msg) => reinhardt_core::exception::Error::Authorization(msg),
			DiError::Authentication(msg) => reinhardt_core::exception::Error::Authentication(msg),
			// Delegate ParamExtraction to the existing `From<ParamError> for
			// CoreError` chain so that structured `ParamErrorContext` (field,
			// expected type, raw value) is preserved and the HTTP status is
			// driven by the inner ParamError variant rather than collapsed
			// to a generic 500.
			#[cfg(feature = "params")]
			DiError::ParamExtraction(inner) => (*inner).into(),
			// `MissingParamContext` is an infrastructure-level misconfiguration
			// (the request boundary did not attach a ParamContext), so it maps
			// to HTTP 500 rather than 400.
			#[cfg(feature = "params")]
			err @ DiError::MissingParamContext { .. } => reinhardt_core::exception::Error::Internal(
				format!("Dependency injection error: {}", err),
			),
