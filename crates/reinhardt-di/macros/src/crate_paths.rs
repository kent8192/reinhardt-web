...
///
/// Mirrors the resolution strategy of [`get_reinhardt_di_crate`] but for
/// `reinhardt-core`. This is used by the `#[injectable]` macro to reach the
/// `async_trait` re-export at `reinhardt_core::async_trait`, so that consumers
/// who depend only on the `reinhardt` facade (and not on `async-trait`
/// directly) can still use DI macros without adding `async-trait` to their
/// `Cargo.toml`. See issue #4445.
///
/// # Resolution order
///
/// 1. Direct `reinhardt-core` dependency (exact match or renamed)
/// 2. Via the `reinhardt` facade crate (`reinhardt::reinhardt_core`)
/// 3. Via the `reinhardt-web` published package name
///    (`reinhardt_web::reinhardt_core`; the package's `[lib] name` is
///    `reinhardt`, so this typically resolves to `reinhardt::reinhardt_core`)
/// 4. Final fallback to `::reinhardt_core`
pub(crate) fn get_reinhardt_core_crate() -> TokenStream {
	...
	// Try via reinhardt-web (published package name; its lib name is `reinhardt`)
	match crate_name("reinhardt-web") {
		Ok(FoundCrate::Itself) => return quote!(crate::reinhardt_core),
		Ok(FoundCrate::Name(name)) => {
			let ident = syn::Ident::new(&name, proc_macro2::Span::call_site());
